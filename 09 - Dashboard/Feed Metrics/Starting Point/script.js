...
async function drawDashboard() {
  ...
  metrics.forEach((metric) => {
    const domain = extent(dataset, (d) => parseInt(d[metric.key]));
  ...
  sections
    ...
    .html((d) =>
      d.note(
        d.scales.qualify(dataset[selectedDay][d.key]),
  ...
  sections
    ...
    .attr("class", "addendum")
    .html((d) =>
      d.addendum(d.formatValue(dataset[selectedDay][d.key]), "Football")
    );
  ...
  bounds
    .append("text")
    .text((d) => d.formatValue(dataset[selectedDay][d.key]))
    .attr("class", "value")
    .attr("text-anchor", "middle")
    .attr("fill", "currentColor")
    .attr("x", dimensions.boundedWidth / 2)
  ...
  groupsGauge
    ...
    .attr("fill", (d) =>
      color(d.scales.fill(dataset[selectedDay][d.key])).darker(1)
    );
